...
export default function CalendarPage() {
  ...
  // Fetch events for a 6-month window centred on today
  ...
  // Map API events → FullCalendar event objects
  ...
  // Derive unique course IDs for the legend
  ...
  function handleEventClick(info: EventClickArg) {
    ...
    const raw = info.event.extendedProps.raw as CalendarEventResponse;
    setSelected(raw);
  }
            ...
            <>
              ...
              <div className="text-sm space-y-2 mt-2">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span>{format(parseISO(selected.start_date), "MMM d, yyyy HH:mm")}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span className="font-medium text-foreground">Ends</span>
                  <span>{format(parseISO(selected.end_date), "MMM d, yyyy HH:mm")}</span>
                </div>
                {selected.course_id && (
                  <div className="flex justify-between text-muted-foreground">
